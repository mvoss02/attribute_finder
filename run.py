...
async def _process_article(article: dict) -> dict:
    ...
    for attribut in article.get("Klassifikations-Attribute", []):
        logger.info(
            f"Analysing article: {product_id} and the corresponding attribute is: {
                attribut.get('Bezeichner')
            }"
        )
        ...
        if len(image_urls) != 0:
            ...
            logger.error(
                f"No image urls where supplied for the following product id: {
                    product_id
                }"
            )
        ...
        if (
            ...
            logger.warning(
                f"Failed to process article: {
                    product_id
                } and the corresponding attribute: {attribut.get('Identifier')}"
            )
