...
def post_json_to_ftp(): # Renamed from previous example to match your original
    """
    ...
    try:
        if not data_config.raw_data_path or not os.path.isdir(data_config.raw_data_path):
            logger.error(f"Error: '{data_config.raw_data_path}' is not a valid directory or is not provided.")
            return # Exit the function if the source directory is invalid

        logger.info(f"Scanning local directory: {data_config.raw_data_path} for JSON files to upload.")
        
        uploaded_count = 0
        for filename in os.listdir(data_config.raw_data_path):
            if filename.endswith('.json'):
                local_file_full_path = os.path.join(data_config.raw_data_path, filename)
                ...
                if os.path.isfile(local_file_full_path):
                    ...
                    logger.warning(f"Skipping '{filename}' in '{data_config.raw_data_path}' as it is not a regular file.")
            else:
                logger.info(f"Skipping '{filename}' as it is not a JSON file.")
        
        if uploaded_count == 0:
            logger.warning(f"No JSON files found in '{data_config.raw_data_path}' to upload.")
        else:
            logger.info(f"Finished uploading {uploaded_count} JSON file(s) from '{data_config.raw_data_path}'.")
