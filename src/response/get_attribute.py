...
#         download_and_process_image,
#         image_url,
#         verify_certificate=response_config.verify_certificate
#     )

#     if not processed_image_path:
#         logger.error(f'Failed to process image from URL: {image_url}')
#         await asyncio.to_thread(write_failed_image, product_id, supplier_colour, image_url)
#         return None

#     try:
#         image_bytes = await asyncio.to_thread(lambda: open(processed_image_path, 'rb').read())
#         base64_image = base64.b64encode(image_bytes).decode('utf-8')

#         logger.info(f'Getting LLM Response from product {product_id} with image {image_url}')
...
#     finally:
#         if processed_image_path and os.path.exists(processed_image_path):
#             await asyncio.to_thread(os.remove, processed_image_path)

# if __name__ == '__main__':
#     logger.info('Starting LLM response directly from __main__')

#     import time
#     start_time = time.time()

#     async def main():
#         test_result_colour = await get_response(
#             product_id=80416852,
#             supplier_colour='8426 Nightshadow Blu',
#             temperature=0.0,
#             categories=[None],
...
#         test_result = await get_response(
#             product_id=80416852,
#             supplier_colour='8426 Nightshadow Blu',
#             temperature=0.0,
#             categories=['turtleneck', 'uBoot', 'envelope', 'troyer', 'henley', 'rollkragen',
#                        'kapuze', 'rundhals', 'karree', 'volant', 'aufliegend', 'stehkragen',
#                        'vNeck', 'schulterfrei', 'rueckenausschnitt', 'wasserfall', 'offen', 'reversekragen'],
#             product_category='Damen / D-Jacken',
#             brand='amberundjune',
#             target_group='D',
#             image_url='https://assets.bettybarclay.com/media/image/c491ec3dd5db9f9fc8b350b627aa74ee80416852-8426-f.jpg',
